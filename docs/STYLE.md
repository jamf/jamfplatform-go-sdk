...
| `methodNotes` | 2 | generated method name → prose appended as the last paragraph of its godoc, after the summary, the deprecation marker and the required-privileges block. `docNotes`' counterpart for facts about the *call* a consumer cannot learn from the spec: an operation the gateway publishes but does not route (403 for every caller whatever their privileges), a server-version floor a declared-required property imposes on the whole request, or a deprecation whose successor the spec does not name. Never enters `api/`. **A key matching no emitted method is a build failure**, and that is the self-expiry — the entry must be deleted by hand when the route lands or the floor lifts, so each note naming a refusal should also name the acceptance test that fails that day. A `methodNotes` entry on a types-only spec is refused outright, since that path emits no methods to attach to. Two users: `jpapi` (the three unrouted v2154 operations plus `UpdateAccountPreferencesV3`'s 11.32 floor) and `capi` (both `/activationcode` verbs) |
...
| `propertyRemovals` | 1 | schema → list of dotted paths to delete. **Panics on a missing path** — a declared removal that can't be walked is a typo. Targets inline objects only: a path resolving through a shared `$ref` affects every schema referencing it. Takes the property's `required` entry with it, so a removal cannot publish a schema requiring a property it does not declare. **Its panic is the wrong self-expiry for a property the *server* rejects** — it fires when the spec drops the path, not when the server catches up, so pair such an entry with an acceptance test that pins the refusal (`AccountPreferencesV6.showDirectoryGroupUuidColumn` is the worked example) |
